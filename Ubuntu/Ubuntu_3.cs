...
{
    public partial class Ubuntu_3 : Form
    {
        public Ubuntu_3()
        {
            InitializeComponent();
            listBox1.SelectedIndex = 82;
        ...
        private void listBox2_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            Brush myBrush = new SolidBrush(Color.Black);
            const int PaddingLeft = 5;
            string text = (string)listBox2.Items[e.Index];
            SizeF textSize = e.Graphics.MeasureString(text, listBox2.Font);
            SolidBrush brush = (isItemSelected) ? new SolidBrush(Color.White) : new SolidBrush(Color.Black);


            e.Graphics.DrawString(text, listBox2.Font, brush,
                e.Bounds.Left + PaddingLeft, e.Bounds.Top );
            e.DrawFocusRectangle();
        }
        ...
        {
            Ubuntu_2 img = new Ubuntu_2() { Dock = DockStyle.Fill, TopLevel = false, TopMost = true };
            this.Controls.Clear();
            img.FormBorderStyle = FormBorderStyle.None;
            this.Controls.Add(img);
            img.Show();
